...
async fn relationships_crud_sqlite_in_memory() {
    ...
    let res = relationships::get_relationship_by_id(
        axum::Extension(arc_state.clone()),
        headers.clone(),
    ...
    .expect("get");
    let got = res.0;
    assert_eq!(got.get("type").and_then(|v| v.as_str()), Some("friend"));

    // update
    let update_body = serde_json::json!({ "type": "colleague" });
    let res = relationships::update_relationship(
        axum::Extension(arc_state.clone()),
        headers.clone(),
        axum::extract::Path(path.clone()),
    ...
    .expect("update");
    let upd = res.0;
    assert_eq!(upd.get("type").and_then(|v| v.as_str()), Some("colleague"));

    // delete
    let res = relationships::delete_relationship(
        axum::Extension(arc_state.clone()),
        headers.clone(),
