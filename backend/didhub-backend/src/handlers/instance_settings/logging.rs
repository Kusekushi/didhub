...
pub async fn get_logging_config(
    ...
    let filter = row.and_then(|r| r.value_string).unwrap_or_else(|| "info".to_string());
...
pub async fn set_logging_config(
    ...
    if let Some(reload) = &state.reload_handle {
        ...
        (reload)(env_filter).map_err(|e| ApiError::internal_error(format!("failed to reload logging: {e}")))?;
    }
