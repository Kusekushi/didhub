...
import { ArrowLeft, Trash2, Pencil, Check, X, Network, Plus, Crown } from 'lucide-react'
...
export default function SubsystemDetail() {
  ...
  const [members, setMembers] = useState<SubsystemMember[]>([])
  ...
  // Check if user can edit this subsystem (owner or admin)
  const canEdit = subsystem?.systemId === authUser?.id || isAdmin
  ...
  const loadSubsystem = async () => {
    ...
    setLoading(true)
    try {
      const response = await api.getSubsystem({ path: { subsystemId } })
      const subsystemData = response.data as Subsystem
      ...
      // Load members with fresh subsystem data
      await loadMembers(subsystemData)
    } catch {
      showToast({ title: 'Error', description: 'Failed to load subsystem details', variant: 'error' })
      navigate('/system')
    } finally {
      setLoading(false)
    }
  ...
  const loadMembers = async (subsystemData?: Subsystem) => {
    const sub = subsystemData || subsystem
    if (!subsystemId || !sub) return
    try {
      // Get members from the subsystem_members table
      const response = await api.listSubsystemMembers({ path: { subsystemId } })
      // Backend returns a plain array
      const memberList = Array.isArray(response.data) ? response.data : []
      setMembers(memberList as SubsystemMember[])
      
      // Now fetch alter details for each member
      const systemId = sub.systemId
      if (systemId && memberList.length > 0) {
        ...
        const memberAlterIds = new Set(memberList.map((m: SubsystemMember) => m.alterId))
  ...
  const handleAddMembers = async () => {
    if (!subsystem || selectedAlterIds.length === 0) return
    
    setAddingMembers(true)
    try {
      // Add each selected alter as a member
      for (const alterId of selectedAlterIds) {
        await api.addSubsystemMember({
          path: { subsystemId: subsystem.id },
          body: { alterId }
        })
      }
      
      showToast({ title: 'Success', description: `Added ${selectedAlterIds.length} member(s) to subsystem` })
  ...
  const handleRemoveMember = async (alterId: string) => {
    ...
    try {
      await api.removeSubsystemMember({
        path: { subsystemId: subsystem.id, memberId: alterId }
      })
  ...
  const handleToggleHost = async (alterId: string, currentIsHost: boolean) => {
    if (!subsystem) return
    
    try {
      await api.updateSubsystemMember({
        path: { subsystemId: subsystem.id, memberId: alterId },
        body: { isHost: !currentIsHost }
      })
      showToast({ 
        title: 'Success', 
        description: currentIsHost ? 'Host status removed' : 'Set as host' 
      })
      await loadMembers()
    } catch {
      showToast({ title: 'Error', description: 'Failed to update host status', variant: 'error' })
    }
  }

  const openAddMemberDialog = async () => {
    if (!subsystem) return
    await loadSystemAlters(subsystem.systemId)
    setSelectedAlterIds([])
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.deleteSubsystem({ path: { subsystemId: subsystem.id } })
      showToast({ title: 'Success', description: 'Subsystem deleted successfully' })
      navigate(`/system/${subsystem.systemId}`)
    } catch {
      showToast({ title: 'Error', description: 'Failed to delete subsystem', variant: 'error' })
    }
  }

  const startEditing = (field: EditableField) => {
    if (!subsystem || !canEdit) return
    
    let initialValue = ''
    switch (field) {
      case 'name':
        initialValue = subsystem.name || ''
        break
      case 'description':
        initialValue = subsystem.description || ''
        break
    }
  ...
  const saveField = async () => {
    if (!subsystem || !editingField) return
    ...
    try {
      ...
      await api.updateSubsystem({
        path: { subsystemId: subsystem.id },
        body: updatePayload
      })
      
      // Update local state
      setSubsystem(prev => prev ? { ...prev, [editingField]: editValue } : null)
      showToast({ title: 'Success', description: 'Subsystem updated successfully' })
      cancelEditing()
    } catch {
      showToast({ title: 'Error', description: 'Failed to update subsystem', variant: 'error' })
    } finally {
      setSaving(false)
    }
  }
  ...
  if (loading) {
    return <div className="p-6">Loading subsystem...</div>
  }

  if (!subsystem) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        <h2 className="text-xl font-semibold mb-2">Subsystem not found</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <Button variant="ghost" onClick={() => navigate(`/system/${subsystem.systemId}`)}>
          <ArrowLeft className="w-4 h-4 mr-2" />
        ...
        <div className="border-t pt-6">
            ...
            {canEdit && (
              <Dialog open={addMemberDialogOpen} onOpenChange={setAddMemberDialogOpen}>
                ...
                <DialogContent className="max-w-2xl">
                  <DialogHeader>
                    <DialogTitle>Add Members to Subsystem</DialogTitle>
                    <DialogDescription>
                      Select alters from this system to add to the subsystem.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="py-4">
          ...
          {members.length > 0 ? (
                ...
                return (
                  <li key={member.alterId} className="flex items-center gap-3 p-3 bg-muted/50 rounded-md">
                    <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center text-sm font-medium relative">
                      ...
                      {member.isHost && (
                        <Crown className="w-4 h-4 absolute -top-1 -right-1 text-yellow-500 fill-yellow-500" />
                      )}
                    </div>
                    ...
                    {canEdit && (
                      <div className="flex items-center gap-1">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleToggleHost(member.alterId, member.isHost)}
                          className={`h-8 w-8 p-0 ${member.isHost ? 'text-yellow-500 hover:text-yellow-600' : 'text-muted-foreground hover:text-yellow-500'}`}
                          title={member.isHost ? 'Remove host status' : 'Set as host'}
                        >
                          <Crown className={`w-4 h-4 ${member.isHost ? 'fill-yellow-500' : ''}`} />
                        </Button>
                        <Button
                          variant="ghost"
                          size="sm"
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>No members in this subsystem yet.</p>
              {canEdit && (
