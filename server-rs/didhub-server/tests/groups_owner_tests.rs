...
async fn nonadmin_cannot_create_for_other_but_can_create_for_self() {
    ...
    if st_forb != StatusCode::FORBIDDEN { panic!("expected forbidden, got {:?} - body: {:?}", st_forb, body_forb); }

    // create group for self (no owner provided)
    let (st_ok, body_ok) = auth_req(&app, axum::http::Method::POST, "/api/groups", &token_a, Some(json!({"name":"SelfGroup"}))).await;
    assert_eq!(st_ok, StatusCode::CREATED, "unexpected status: {:?} - body: {:?}", st_ok, body_ok);
    assert_eq!(body_ok["owner_user_id"].as_i64().unwrap(), user_a.id);

    // create group with explicit owner = self
    let (st_ok2, body_ok2) = auth_req(&app, axum::http::Method::POST, "/api/groups", &token_a, Some(json!({"name":"SelfGroup2","owner_user_id": user_a.id}))).await;
    assert_eq!(st_ok2, StatusCode::CREATED, "unexpected status: {:?} - body: {:?}", st_ok2, body_ok2);
