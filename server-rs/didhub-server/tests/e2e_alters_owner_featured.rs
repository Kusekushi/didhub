...
async fn e2e_alter_owner_behavior_featured() {
    ...
    let (status, body) = test_utils::auth_req(
        &app,
    ...
    assert!(
        status.is_success(),
        "admin create alter failed: {:?}",
        (status, body)
    );
    if let Some(owner) = body.get("owner_user_id") {
        assert_eq!(owner.as_i64().unwrap(), 2);
    }

    // verify audit entry exists for alter.create
    let audits = db
        .list_audit(Some("alter.create"), None, None, None, 10, 0)
        .await
        .expect("list audit");
    assert!(!audits.is_empty(), "no audit rows for alter.create");
    let found = audits
        .iter()
        .any(|a| a.action == "alter.create" && a.user_id == Some(1));
    ...
    let (status2, _body2) = test_utils::auth_req(
        &app,
        axum::http::Method::POST,
        "/api/alters",
        &user_token,
