...
async fn memory_cache_ttl() {
    let cache = AppCache::memory();
    let v = Sample { a: 1, b: "test".into() };
    
    // Set with TTL
    cache.set("ttl_key", &v, Some(Duration::from_millis(50))).await.unwrap();
...
async fn memory_cache_no_ttl() {
    let cache = AppCache::memory();
    let v = Sample { a: 100, b: "persistent".into() };
...
async fn memory_cache_incr_with_ttl() {
    ...
    let val1 = cache.incr("incr_ttl", Some(Duration::from_millis(50))).await.unwrap();
    assert_eq!(val1, 1);
    
    let val2 = cache.incr("incr_ttl", Some(Duration::from_millis(50))).await.unwrap();
    assert_eq!(val2, 2);
...
async fn memory_cache_complex_types() {
    ...
    let complex = ComplexSample {
        ...
        nested: Some(Box::new(Sample { a: 99, b: "nested".to_string() })),
    };
...
async fn memory_cache_overwrite() {
    ...
    let v1 = Sample { a: 1, b: "first".into() };
    let v2 = Sample { a: 2, b: "second".into() };
...
async fn memory_cache_incr_edge_cases() {
    ...
    cache.set("string_key", &"not_a_number", None).await.unwrap();
...
async fn memory_cache_cleanup() {
    ...
    for i in 0..10 {
        cache.set(&format!("temp_{}", i), &i, Some(Duration::from_millis(10))).await.unwrap();
    }
