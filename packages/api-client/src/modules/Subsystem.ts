...
function normalizeSubsystem(subsystem: unknown): Subsystem {
  if (!subsystem) return subsystem as Subsystem;
  const s = subsystem as Record<string, any>;
  return {
    ...s,
...
export async function listSubsystems(q = '', ownerUserId?: string, includeMembers = false): Promise<any[]> {
  ...
  if (q && q.startsWith('?')) {
    const qs = q;
    return apiFetch('/api/subsystems' + qs + fieldsQuery).then((r: ApiFetchResult) => {
      const j = r.json || {};
      if (Array.isArray(j)) return j.map(normalizeSubsystem);
      if (j && Array.isArray((j as any).items)) return (j as any).items.map(normalizeSubsystem);
      return [] as any[];
    });
  }
...
export async function getSubsystem(id: string | number): Promise<Subsystem | null> {
  return apiFetch('/api/subsystems/' + id).then((r: ApiFetchResult) => normalizeSubsystem(r.json) || null);
}

export async function createSubsystem(payload: Record<string, any>): Promise<ApiFetchResult> {
...
export async function updateSubsystem(id: string | number, payload: Record<string, any>): Promise<ApiFetchResult> {
...
export async function listSubsystemMembers(id: string | number): Promise<SubsystemMember[]> {
  return apiFetch('/api/subsystems/' + id + '/members').then((r: ApiFetchResult) => r.json || {});
}
...
export async function toggleSubsystemLeader(id: string | number, alterId: string | number, add = true): Promise<Record<string, unknown>> {
  return apiFetch('/api/subsystems/' + id + '/leaders/toggle', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function migrateAlterSubsystems(): Promise<Record<string, unknown>> {
  return apiFetch('/admin/migrate-alter-subsystems', { method: 'POST' }).then((r: any) => r.json || {});
}
