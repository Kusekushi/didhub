...
export async function fetchAltersBySystem(uid: string, includeRelationships = false): Promise<Alter[] | ApiFetchResultError> {
  ...
  const r = await apiFetch('/api/alters?' + params.toString());
  if (!r || typeof r !== 'object' || r.status !== 200) return { status: r.status ?? 404 } as ApiFetchResultError;
  return ((r.json as any)?.items || []).map(normalizeAlter);
}
...
export async function fetchAlters(q = '', includeRelationships = false): Promise<{ items?: any[] } | any> {
  const qs: string[] = [];
  ...
  return apiFetch('/api/alters' + query).then((r: ApiFetchResult) => {
    const j = r.json || {};
    if (j.items) j.items = j.items.map(normalizeAlter);
    return j;
  });
}

export async function getAlter(id: string | number): Promise<Alter> {
  return apiFetch('/api/alters/' + id).then((r: ApiFetchResult) => normalizeAlter(r.json) || null);
}
...
export async function deleteAlterImage(alterId: string | number, url: string): Promise<Record<string, unknown>> {
  return apiFetch('/api/alters/' + alterId + '/image', {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchAlterNames(q = ''): Promise<AlterName[]> {
  return apiFetch('/api/alters/names' + (q ? '?q=' + encodeURIComponent(q) : '')).then(
...
export async function fetchAlterNamesByUser(user_id: string | number | undefined, q = ''): Promise<AlterName[]> {
  ...
  return apiFetch('/api/alters/names' + qs).then((r: ApiFetchResult) => r.json || {});
}

export async function fetchFamilyTree(): Promise<FamilyTreeResponse> {
  return apiFetch('/api/alters/family-tree').then((r: ApiFetchResult) => r.json || null);
}
...
export async function listUserAlterRelationships(alterId: number): Promise<any[]> {
  return apiFetch(`/api/alters/${alterId}/relationships`).then((r: ApiFetchResult) => r.json || []);
}
