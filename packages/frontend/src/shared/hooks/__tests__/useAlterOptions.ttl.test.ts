...
describe('useAlterOptions cache TTL', () => {
  ...
  vi.mock('../../../services/alterService', () => ({
    searchAlters: (...args: any[]) => searchMock(...args),
  }));
  ...
  it('reuses cached results for identical queries within TTL', async () => {
    ...
    searchMock.mockResolvedValue({ items: [{ id: 'v1' }], total: 1, limit: 20, offset: 0 });
    ...
    searchMock.mockResolvedValue({ items: [{ id: 'v2' }], total: 1, limit: 20, offset: 0 });
